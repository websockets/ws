...
class WebSocket extends EventEmitter {
   ...
   *
   * @param {(Boolean|Error|undefined)} error Indicates whether or not an error
   *     occurred
   * @private
   */
  finalize (error) {
    ...
    this._receiver.cleanup(() => {
      if (typeof error === 'object' && !this._receiver._hadError) {
        //
        // Re-emit the `net.Socket` error if an `'error'` event has not already
        // been emitted.
        //
        this.emit('error', error);
      }
