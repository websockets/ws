...
WebSocket.prototype.close = function(code, data) {
  if (this._readyState == WebSocket.CLOSING) return;
  if (this._readyState == WebSocket.CONNECTING) {
    this._readyState = WebSocket.CLOSED;
    return;
  }
  if (this._readyState != WebSocket.OPEN) throw new Error('not opened');
  try {
...
WebSocket.prototype.stream = function(options, cb) {
  ...
  var send = function(data, final) {
    try {
      if (self._readyState != WebSocket.OPEN) throw new Error('not opened');
...
function upgrade(res, socket, upgradeHead) {
  ...
  socket.on('end', function() {
    if (self._readyState == WebSocket.CLOSED) return;
