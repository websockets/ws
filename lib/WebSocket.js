...
WebSocket.prototype.emitOpen = function emitOpen() {
  this.emit('open', new OpenEvent(this));
};

WebSocket.prototype.emitMessage = function emitMessage(data, flags) {
  if (flags.binary && this._binaryType === 'arraybuffer') {
    data = new Uint8Array(data).buffer;
  }
  this.emit('message', new MessageEvent(data, !!flags.binary, this));
};

WebSocket.prototype.emitError = function emitError(event) {
  event.type = 'error';
  event.target = this;
  this.emit('error', event);
};

WebSocket.prototype.emitClose = function emitClose(code, message) {
  this.emit('close', new CloseEvent(code, message, this));
};
...
function initAsClient(address, protocols, options) {
  ...
  req.once('upgrade', function upgrade(res, socket, upgradeHead) {
    ...
    if (typeof serverKey === 'undefined' || serverKey !== expectedServerKey) {
      self.emitError(new Error('invalid server key'));
      self.removeAllListeners();
      socket.end();
      return;
    }
    ...
    if (protError) {
      self.emitError(new Error(protError));
      self.removeAllListeners();
      socket.end();
      return;
    } else if (serverProt) {
    ...
    if (perMessageDeflate && serverExtensions[PerMessageDeflate.extensionName]) {
      try {
        ...
        self.emitError(new Error('invalid extension parameter'));
        self.removeAllListeners();
        socket.end();
        return;
      }
  ...
  stream.on('data', function incoming(data) {
    if (instance.readyState !== WebSocket.OPEN) {
      if (typeof cb === 'function') cb(new Error('not opened'));
        ...
        delete instance._queue;
        instance.emitError(new Error('not opened'));
      }
      return;
    }
...
function cleanupWebsocketResources(error) {
  ...
  this.emitClose(this._closeCode || 1000, this._closeMessage || '');
